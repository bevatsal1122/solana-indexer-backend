...
// Process webhook data directly (synchronous mode)
export async function processWebhookDataSync(
  ...
  try {
    ...
    await supabase.from('logs').insert({
      job_id: jobInfo.id,
      message: `Error processing ${jobType}: ${
...
// Process webhook data for a specific job (asynchronous queue processing)
export async function processWebhookData(
  ...
  return processWebhookDataSync(webhookData, jobInfo, jobType);
}

// Create workers for each queue
export function initializeWorkers() {
  if (!ENABLE_BULL_MQ || !redisConnection) {
    console.warn('⚠️ BullMQ workers initialization skipped - Redis not available');
    return null;
  }
  ...
  try {
    ...
    const nftMintWorker = new Worker('nft-mint-queue', processWebhookData, workerOptions);

    const nftSaleWorker = new Worker('nft-sale-queue', processWebhookData, workerOptions);

    const nftListingWorker = new Worker('nft-listing-queue', processWebhookData, workerOptions);

    const compressedNftMintWorker = new Worker('compressed-nft-mint-queue', processWebhookData, workerOptions);

    // Set up event listeners for the workers
    [nftMintWorker, nftSaleWorker, nftListingWorker, compressedNftMintWorker].forEach(worker => {
      worker.on('completed', (job) => {
        console.log(`Job ${job.id} completed for queue ${job.queueName}`);
      });

      worker.on('failed', (job, err) => {
        console.error(`Job ${job?.id} failed for queue ${job?.queueName}:`, err);
      });
    });
    ...
    console.warn('⚠️ Workers initialization failed - falling back to synchronous processing');
    return null;
  }
} 